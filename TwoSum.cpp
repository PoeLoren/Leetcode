...
int cmp(const void *p,const void *q )
	{
		return (*(number *)p).n >(*(number *)q).n ?1:-1;
	}
...
public:
    ...
    vector<int> twoSum(vector<int> &numbers, int target) {
		...
		qsort(a,numbers.size(),sizeof(a[0]),cmp);
		int i = 0,j = numbers.size()-1;
		while(i < j)
		{
			if(a[i].n + a[j].n == target) 
			{
				s.push_back(a[i].index);
				s.push_back(a[j].index);
				break;
			}
			else if(a[i].n + a[j].n < target)
				i++;
			else
				j--;
		}
